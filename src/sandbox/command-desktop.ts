...
function previewHtml(jpeg?: Uint8Array): string {
  const body = jpeg && jpeg.byteLength
    ...
    : '<main>Sandbox browser preview is ready.</main>';
  return `<!doctype html><html><head><meta charset="utf-8"><style>body{margin:0;background:#0f172a;color:#d6e2ff;font:14px system-ui,sans-serif}header{padding:10px 12px;border-bottom:1px solid #263449}main{padding:16px}img{display:block;max-width:100%;height:auto;margin:auto}</style></head><body><header>Sandbox browser preview</header>${body}</body></html>`;
}
...
if ! command -v node >/dev/null 2>&1; then
  echo "node is required for sandbox browser tools" >&2
  exit 127
fi
...
if [ -z "$CHROME_BIN" ]; then
  echo "chromium/google-chrome is required for sandbox browser tools" >&2
  exit 127
fi
if ! command -v Xvfb >/dev/null 2>&1; then
  echo "Xvfb is required for headed sandbox browser tools" >&2
  exit 127
fi
if [ -z "$DISPLAY" ]; then
  DISPLAY=":99"
...
class CommandDesktopHandle implements DesktopHandle {
  ...
  constructor(
    ...
    private readonly useSandbox: <T>(action: () => Promise<T>) => Promise<T>,
  ) {}
  ...
  async screenshot(): Promise<Uint8Array> {
    ...
    if (!line) throw new Error(`sandbox browser screenshot did not return image data\n${execDetails(res)}`);
    const jpeg = Buffer.from(line.slice(SCREENSHOT_MARKER.length), 'base64');
    this.stream = `data:text/html;charset=utf-8,${encodeURIComponent(previewHtml(jpeg))}`;
    return jpeg;
  }
...
export class CommandDesktopProvider implements DesktopProvider {
  ...
  async create(spec: DesktopSpec): Promise<DesktopHandle> {
    ...
    return new CommandDesktopHandle(
      handle,
      (action) => this.manager.use(spec.key, action),
    );
  }
