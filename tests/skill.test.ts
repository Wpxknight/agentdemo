import { chmod, cp, mkdtemp, mkdir, readFile, readdir, rename, stat, utimes, writeFile } from 'node:fs/promises';
...
describe('SkillRegistry', () => {
  ...
  it('load_skill returns full body and lists bundled files', async () => {
    ...
    const res = await tool.execute({ name: 'inspect' }, { sessionId: 's1', tenantId: 'default', userId: 'u', role: 'user' });
    expect(res.content).toContain('# 巡检步骤');
    expect(res.content).toContain('helper.sh');

    const miss = await tool.execute({ name: 'nope' }, { sessionId: 's1', tenantId: 'default', userId: 'u', role: 'user' });
  ...
  it('disables, enables, and deletes skills without exposing disabled skills to load_skill', async () => {
    ...
    const disabled = await reg.tool().execute({ name: 'inspect' }, { sessionId: 's1', tenantId: 'default', userId: 'u', role: 'user' });
...
describe('SkillRegistry governed Pi loading', () => {
  ...
  it('filters actual product records before every Pi loader call', async () => {
    ...
    expect((await reg.listLoadedFor(viewer)).map((skill) => skill.name)).toEqual(['allowed']);
    const loaded = await reg.tool().execute({ name: 'allowed' }, { sessionId: 's', ...viewer });
