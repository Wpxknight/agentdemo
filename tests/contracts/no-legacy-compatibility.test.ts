...
describe('new-project compatibility boundary', () => {
  ...
  it('uses one baseline migration without legacy runtime markers', async () => {
    const directory = resolve(root, 'src/db/migrations');
    const migrations = (await readdir(directory))
      .filter((name) => name.endsWith('.sql'))
      .sort();

    expect(migrations).toEqual(['0001_baseline.sql']);
    const baseline = await readFile(join(directory, migrations[0]!), 'utf8');
    expect(baseline).not.toMatch(/legacy-v1|compat-v1|langgraph/i);
  });
});
