...
- [x] S6+ 定时任务完整化（2026-07-06）— Store 增 getScheduledTask/updateScheduledTask（cron 变更重算 nextRunAt）/deleteScheduledTask（连带 task_runs，memory+mysql）；HTTP 增 PATCH /v1/schedule/:id（改 cron/task/enabled/preApproved，预批准需 approve 权限）、DELETE /v1/schedule/:id、POST /v1/schedule/:id/run（异步立即执行、进程内去重 409、结果写 task_runs）；会话工具补 update_scheduled_task/delete_scheduled_task；前端定时任务页落地创建表单（cron 预设+自定义+预批准勾选，UTC 提示）与任务操作（立即执行+结果轮询/启停/编辑/删除确认/刷新）；276 测试通过，dev 集群端到端验证（创建→PATCH→立即执行→success 落库→停用→删除连带清 runs）
- [x] S3+ MCP 管理完整化（2026-07-06）— McpManager 运行期生命周期（add/remove/reconnect、按 server 状态跟踪 connected/error+错误信息、失败保留可重连）；MCP server 配置持久化（tenant_settings `mcp.servers`，启动时优先于 config.jsonc，重启自动重连）；HTTP 管理 API（GET/POST /v1/mcp/servers、DELETE、/reconnect，zod 校验 + tenant:manage，工具注册表同步增删）；前端 MCP 页真实数据（server 列表/行选择/新增表单 stdio+sse+http/重连/删除确认/搜索过滤/测试调用）；Dockerfile 打入 scripts/ 供集群内 stdio 冒烟；270 测试通过，dev 集群端到端验证（新增→调用→重启持久化恢复→重连）
