...
- Response：`200 {config,options?}`。config 为 `id,protocol,base_url,model,api_key,api_key_set,api_key_preview,context_window_tokens,context_keep_images,effort?`。注意：当前实现 **会返回完整 `api_key`**；调用方必须按敏感数据处理。
- 错误：`403` 权限。
...
- Body：可用 `id/model_id` 选 option；或 `protocol` (`anthropic|openai`)、`base_url/baseURL`、`api_key/apiKey`、`model`、`context_window_tokens/contextWindowTokens` 正整数、`context_keep_images/contextKeepImages` >=0、`effort`。未知 effort 当前静默保留旧值。
