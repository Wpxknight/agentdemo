---
name: aios-request
description: 查看、查询或操作 AIOS 平台时必须优先使用本技能（走平台 API，而不是用浏览器打开页面）。只要用户提到 AIOS 平台，或给出平台地址、账号密码并要求查看报表、统计数据、管理资源或任务，即视为命中。能力：资源使用报表、任务统计、用户活跃度、资源管理、推理任务、模型训练（算法/通用/YOLO/LSTM）、大模型微调、平台功能引导。关键词：AIOS、报表、资源使用、用户报表、状态查询、训练、推理、微调。
---
